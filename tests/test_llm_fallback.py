...
def test_fallback_script_generation(tmp_path: Path) -> None:
    async def run() -> None:
        ...
        await repo.init()
        script = await ScriptGenerator(settings, repo).generate_script("безопасность", template=None)
...
def test_fallback_idea_generation(tmp_path: Path) -> None:
    async def run() -> None:
        settings = Settings(SQLITE_PATH=tmp_path / "test.sqlite3", GEMINI_API_KEY="")
        repo = HistoryRepository(settings.sqlite_path)
        await repo.init()
        idea = await ScriptGenerator(settings, repo).generate_idea()
        assert idea
